...
class Transacoes:
    ...
    def atualizar_preco_medio(self, custo_unitario, quantidade):
        logging.info(
            f"Atualizando preço médio: custo_unitario={custo_unitario}, \
            quantidade={quantidade}"
        )                
        custo_unitario = ValorMonetario(custo_unitario).valor_monetario
        quantidade = Quantidade(quantidade).quantidade
        logging.info(
            f"Antigo: custo_total={self.custo_total},\
            quantidade_total={self.quantidade_total}"
        )        
        self.preco_medio_calculado = self.calcular_preco_medio(
        ...
        logging.info(
            f"Novo: custo_total={self.custo_total}, \
            quantidade_total={self.quantidade_total}"
        )

    def calcular_preco_medio(
        self, valor_acao_comprada, quantidade_acao_comprada
    ):
        logging.info("Calculado preço médio")
        logging.info(f"Quantidade total atual {self.quantidade_total}")
        logging.info(f"Preco médio calculado {self.preco_medio_calculado}")
        logging.info(f"Quantidade ação comprada {quantidade_acao_comprada}")
        logging.info(f"Valor ação comprada {valor_acao_comprada}")
        preco_medio = ValorMonetario(
            (
                (self.quantidade_total * self.preco_medio_calculado)
                + (quantidade_acao_comprada * valor_acao_comprada)
            )
            / (self.quantidade_total + quantidade_acao_comprada)
        ).valor_monetario
    ...
    def obteve_lucro(self, preco_venda):
        return (
            ValorMonetario(preco_venda).valor_monetario
            > self.preco_medio_calculado
        )
    ...
    def decrementar_quantidade(self, quantidade):
        logging.info(f"Quantidade total antes decrementar: \
            {self.quantidade_total}")
        self.quantidade_total = (
            self.quantidade_total - Quantidade(quantidade).quantidade
        )
        logging.info(f"Quantidade total após decrementar: \
            {self.quantidade_total}")
    ...
    def calcular_valor_imposto(self):
        logging.info(
            f"Calculando imposto sobre lucro total de: {self.lucro_total}"
        )
    ...
    def calcular_lucro(self, preco_venda, quantidade):
        ...
        lucro = ValorMonetario(
            (preco_venda - self.preco_medio_calculado) * quantidade
        ).valor_monetario
        logging.info(
            f"Calculando lucro: \
            preco_venda={preco_venda}, \
            quantidade={quantidade}, \
            preco_medio={self.preco_medio_calculado}, \
            lucro={lucro}"
        )
        return lucro
